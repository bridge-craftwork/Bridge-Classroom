...
// =====================================================================
// time_taken_ms backfill (issue #7 — TEMPORARY)
// =====================================================================
//
// One-shot pass that decrypts every observation whose `time_taken_ms`
// is null, extracts the per-board total from the plaintext blob, and
// stamps the column. Mirrors the lift-from-blob-to-clear-column
// pattern used for issue #15's assignment-context backfill, but here
// the source field really IS in the blob, so the work actually
// recovers data instead of just proving a negative.
//
// Rule for total per-play time, matched to the frontend's extractor in
// `src/utils/observationSchema.js::computeTotalTimeMs`:
//   1. If blob.prompts[] has any entries with a numeric time_ms, use
//      the sum.
//   2. Else use blob.result.time_taken_ms.
//   3. Else leave the column null.
//
// After running once on prod the entire section + the startup hook
// can be deleted.

#[derive(Debug, Default, Serialize)]
pub struct TimeTakenBackfillStats {
    pub users_seen: usize,
    pub users_skipped_no_key: usize,
    pub rows_processed: usize,
    pub rows_stamped: usize,
    pub rows_no_time: usize,
...
#[derive(sqlx::FromRow)]
struct TimeBackfillRow {
    id: String,
    encrypted_data: String,
    iv: String,
}

/// Sum prompts[].time_ms when present; else result.time_taken_ms; else None.
fn extract_time_taken_ms(blob: &serde_json::Value) -> Option<i64> {
    if let Some(arr) = blob.get("prompts").and_then(|v| v.as_array()) {
        let mut total: i64 = 0;
        let mut any = false;
        for p in arr {
            ...
            } else if let Some(t) = p.get("time_ms").and_then(|v| v.as_f64()) {
                if t.is_finite() && t >= 0.0 {
                    total += t.round() as i64;
                    any = true;
                }
            }
        }
    ...
    if let Some(t) = blob.get("result").and_then(|r| r.get("time_taken_ms")).and_then(|v| v.as_i64()) {
        if t >= 0 {
            return Some(t);
        }
    }
    if let Some(t) = blob.get("result").and_then(|r| r.get("time_taken_ms")).and_then(|v| v.as_f64()) {
        if t.is_finite() && t >= 0.0 {
            return Some(t.round() as i64);
        }
    }
    None
}

/// Walk every user's still-unstamped observations, decrypt each blob,
/// and write `time_taken_ms`. Idempotent — only touches rows where
/// `time_taken_ms IS NULL`.
pub async fn backfill_time_taken_ms(
    pool: &sqlx::SqlitePool,
    recovery_secret: &str,
) -> Result<TimeTakenBackfillStats, String> {
    let mut stats = TimeTakenBackfillStats::default();

    let pending: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM observations WHERE time_taken_ms IS NULL",
    )
    .fetch_one(pool)
    .await
    ...
    if pending == 0 {
        tracing::info!("time_taken_ms backfill: nothing to do");
        return Ok(stats);
    }
    tracing::info!("time_taken_ms backfill: starting with {} unstamped observations", pending);
    ...
    for user in &users {
        ...
        let rows: Vec<TimeBackfillRow> = sqlx::query_as(
            r#"
            SELECT id, encrypted_data, iv
            FROM observations
            WHERE user_id = ? AND time_taken_ms IS NULL
            "#,
        )
        .bind(&user.id)
        .fetch_all(pool)
        ...
        for obs in rows {
            ...
            let time_ms = extract_time_taken_ms(&blob);
            if time_ms.is_none() {
                stats.rows_no_time += 1;
                // Leave the column null; we'll retry on next restart.
                // (We could stamp 0 to stop retrying, but null
                // distinguishes "unknown" from "took zero ms.")
                continue;
            }

            match sqlx::query(
                "UPDATE observations SET time_taken_ms = ? WHERE id = ?",
            )
            .bind(time_ms)
            .bind(&obs.id)
            .execute(pool)
            .await
            {
                Ok(_) => stats.rows_stamped += 1,
                Err(e) => {
                    tracing::warn!("Update failed for obs {}: {}", obs.id, e);
    ...
    tracing::info!("time_taken_ms backfill complete: {:?}", stats);
    Ok(stats)
}
