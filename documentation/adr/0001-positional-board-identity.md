...
**Status:** Proposed (→ Accepted on sign-off by Rick and David)
**Date:** 2026-07-05
...
4. **Readiness — record, but flag `prerelease`.** The producer marks boards `ready` in the PBN (file-level default, per-board override; absent = not ready). Not-ready boards are fully playable and **their observations are recorded**, each stamped with a consumer-side **`prerelease`** flag — the inverse of the board's `ready` state (`observations.prerelease = NOT board.ready`). Prerelease observations are **excluded from all mastery *and* platform statistics**, but **kept for the student's own history, navigation, and drill-down** and shown with a distinct marker (a triangle rather than a circle). They **cannot be selected into exercises**. **A replacement for a ready board must itself be `ready=true`.** (`prerelease` is deliberately named apart from the producer's `ready` flag so the two concepts don't blur: `ready` is a property of the board in the repo; `prerelease` is a property of a recorded observation.)

5. **Skill-path mapping.** Every board maps to a real skill path; no `uncategorized`. New paths are minted as needed. (Sequenced after the `collection_id` + `prerelease` backfill, so those become the durable discriminators before the `uncategorized` marker is remapped away.)
