...
5. **Skill-path mapping.** Every board maps to a real skill path; no `uncategorized`. New paths are minted as needed. (Sequenced after the collection key and the content deletion, so the discriminator is not lost mid-migration.)
